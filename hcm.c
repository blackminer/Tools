...
static struct option longopts[] =
{
 /* { *name has_arg *flag val } */
    {"usb-address",         required_argument, 0, 'a'},                 // USB address for device select
    {"usb-bus",             required_argument, 0, 'b'},                 // USB Bus for device select
#if defined(LIBUSBX_API_VERSION) && (LIBUSBX_API_VERSION >= 0x01000102)
    {"usb-port",            required_argument, 0, 'p'},                 // USB Port for device select
#endif
...
static int receive_frame(hcm_usb_t *s, uint8_t *buf, int bufsize, int expected)
    ...
    // There seems to be a fetish with generating OP_USB_NOTICE's. Also skip any status frames that come our way.
    ...
    h = (struct hf_header *)&crap[0];

    for (i = 0, got = 0; i < 10000; i++)
        {
        sts = libusb_bulk_transfer(s->handle, s->receive_endpoint->bEndpointAddress, &crap[got], sizeof(crap)-got, &rxbytes, 0);
        if (sts != 0)
            {
